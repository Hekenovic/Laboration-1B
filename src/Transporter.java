...
public class Transporter extends Car implements Loader {
    ...
    private ArrayList<Car> carLoadedList = new ArrayList<>();
    ...
    public void unload() {
        if (!isRampUp() && carLoadedList.size() != 0) {
            ...
            carBeingRemoved.setXPosition(getXPosition() - 2);
            carBeingRemoved.setYPosition(getYPosition());
            carCounter--;
        } else
            throw new RuntimeException("Requirements to load are not met.");
     ...
     */
    public ArrayList<Car> getCarLoadedList() {
